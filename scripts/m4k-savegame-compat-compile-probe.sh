...
for required in "${source_probe}" "${m4k_patcher}"; do
  if [[ ! -f "${required}" ]]; then
    echo "error: M4K compile prerequisite is missing: ${required}" >&2
    exit 2
  fi
done
...
driver="${workdir}/m4k-savegame-compat-compile-probe.sh"
...
if text.count(variable_marker) != 1:
    raise SystemExit("M4K predecessor patcher-variable marker changed")
text = text.replace(variable_marker, variable_replacement, 1)

required_marker = 'for required in "${source_probe}" "${m4j_patcher}"; do\n'
required_replacement = (
    'for required in "${source_probe}" "${m4j_patcher}" "${m4k_patcher}"; do\n'
)
if text.count(required_marker) != 1:
    raise SystemExit("M4K predecessor required-input marker changed")
...
if text.count(patch_marker) != 1:
    raise SystemExit("M4K predecessor patch-chain marker changed")
...
text = text.replace(
    ...
    "generated-m4k-savegame-compat-compile-driver.sh",
)
text = text.replace(
    "m4j-savegame-context-compile-probe:",
    "m4k-savegame-compat-compile-probe:",
)
