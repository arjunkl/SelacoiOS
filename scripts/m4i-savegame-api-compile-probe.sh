...
zscript_diagnostics_patcher="${repo_root}/scripts/patch-gzselaco-zscript-diagnostics-ios-m4e.py"
retail_savegame_patcher="${repo_root}/scripts/patch-gzselaco-savegame-virtuals-ios-m4f.py"
override_signature_patcher="${repo_root}/scripts/patch-gzselaco-override-signature-diagnostics-ios-m4g.py"
retail_signature_patcher="${repo_root}/scripts/patch-gzselaco-savegame-signatures-ios-m4h.py"
savegame_api_diagnostics_patcher="${repo_root}/scripts/patch-gzselaco-savegame-api-diagnostics-ios-m4i.py"
...
evidence_dir="${repo_root}/build/evidence/m4i-savegame-api-compile"
rm -rf "${evidence_dir}"
...
for required_file in \
  ...
  "${zscript_diagnostics_patcher}" \
  "${retail_savegame_patcher}" \
  "${override_signature_patcher}" \
  "${retail_signature_patcher}" \
  "${savegame_api_diagnostics_patcher}" \
  "${runtime_overlay}" \
  "${swapchain_overlay}" \
  "${engine_init_overlay}" \
  ...
  if [[ ! -f "${required_file}" ]]; then
    echo "error: Milestone 4I compile input is missing: ${required_file}" >&2
    exit 2
  fi
done
...
driver="${workdir}/m4i-savegame-api-compile-driver.sh"
...
cp "${driver}" "${evidence_dir}/generated-m4i-savegame-api-compile-driver.sh"
bash -n "${driver}"
bash "${driver}"
