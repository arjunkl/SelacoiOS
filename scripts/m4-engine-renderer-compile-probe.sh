...
evidence_dir="${repo_root}/build/evidence/m4-engine-renderer-compile"
rm -rf "${evidence_dir}"
...
for required_file in \
  ...
  "${swapchain_overlay}" \
  "${engine_init_overlay}" \
  "${engine_renderer_overlay}"; do
  if [[ ! -f "${required_file}" ]]; then
    echo "error: Milestone 4B compile input is missing: ${required_file}" >&2
    exit 2
  fi
done
...
driver="${workdir}/m4-engine-renderer-compile-driver.sh"
...
text = text.replace(
    ...
    "build/evidence/m4-engine-renderer-compile",
)
...
cp "${driver}" "${evidence_dir}/generated-engine-renderer-compile-driver.sh"
bash -n "${driver}"
bash "${driver}"
