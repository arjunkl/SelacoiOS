...
for required_file in \
  ...
  if [[ ! -f "${required_file}" ]]; then
    echo "error: Milestone 3 compile input is missing: ${required_file}" >&2
    exit 2
  fi
done
...
driver="${workdir}/m3-local-asset-compile-driver.sh"
...
cp "${driver}" "${evidence_dir}/generated-local-asset-compile-driver.sh"
bash -n "${driver}"
bash "${driver}"
