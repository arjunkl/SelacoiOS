...
evidence_dir="${repo_root}/build/evidence/m4-engine-init-compile"
rm -rf "${evidence_dir}"
...
for required_file in \
  ...
  "${runtime_overlay}" \
  "${swapchain_overlay}" \
  "${engine_init_overlay}"; do
  if [[ ! -f "${required_file}" ]]; then
    echo "error: Milestone 4 compile input is missing: ${required_file}" >&2
    exit 2
  fi
done
...
driver="${workdir}/m4-engine-init-compile-driver.sh"
...
cp "${driver}" "${evidence_dir}/generated-engine-init-compile-driver.sh"
bash -n "${driver}"
bash "${driver}"
